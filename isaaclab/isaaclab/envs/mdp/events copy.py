...
def reset_goal_position(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    center_position: tuple[float, float] = (12.0, 12.0),
    env_spacing: float = 16.0,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("goal"),
):
    """환경마다 목표 지점의 위치를 초기화합니다.

    Args:
        env: 환경 객체
        env_ids: 초기화할 환경들의 인덱스
        center_position: 첫 번째 환경의 기준 목표 위치
        env_spacing: 환경 간 간격 (x축 방향으로만 고려)
        asset_cfg: 목표 지점 asset의 설정
    """
    from omni.isaac.core.prims import XFormPrimView
    # 목표 지점은 일반적으로 XFormPrimView에 의해 관리됨
    goal: XFormPrimView = env.scene[asset_cfg.name]

    num_envs_total = env.num_envs
    grid_width = int(math.sqrt(num_envs_total))  # 예: 4096 -> 64 x 64
    positions = torch.zeros((env_ids.shape[0], 3), device=env.device)

    # 각 환경마다 목표 위치 계산
    for i, env_id in enumerate(env_ids):
        idx = env_id.item()
        x_idx = idx % grid_width
        y_idx = idx // grid_width

        positions[i, 0] = center_position[0] + x_idx * env_spacing
        positions[i, 1] = center_position[1] + y_idx * env_spacing
        positions[i, 2] = 0.0  # 바닥 고정

    # 위치 적용
    goal.set_world_poses(positions, env.scene[asset_cfg.name].get_world_orientations(env_ids), env_ids=env_ids)
