...
from isaaclab_assets.robots.husky import HUSKY_CFG
...
@configclass
class HuskyEnvCfg(DirectRLEnvCfg):
    ...
    robot_cfg: ArticulationCfg = HUSKY_CFG.replace(
...
class HuskyEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: HuskyEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.rew_scale_goal  = 12.0 
        self.rew_scale_head  = 0.1
        self.goal_reached = torch.zeros((self.num_envs), device=self.device, dtype=torch.float32)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # 환경 원점에 대해 초기화 & 0.3 이내로 랜덤화
        root_pose[:, :3] += self.scene.env_origins[env_ids]
        root_pose[:, 0] += torch.empty(len(env_ids), device=self.device).uniform_(-0.3, 0.3)
        root_pose[:, 1] += torch.empty(len(env_ids), device=self.device).uniform_(-0.3, 0.3)

        # 초기 각도 랜덤화(±30°)
        rand_yaw = (torch.rand(len(env_ids), device=self.device)+1) * 0.6
