...
class TurtlebotEnvCfg(DirectRLEnvCfg):
    ...
    R_WHEEL   = 0.033   # 바퀴 반경 [m]
    WHEELBASE = 0.160   # 좌·우 바퀴 간 거리 [m]

    MAX_LIN   = 0.5    # m/s
    MAX_ANG   = 2.84    # rad/s
    MAX_OMEGA = 26.0    # 바퀴 자체 각속도(rad/s) 상한 (= velocity_limit)
...
class TurtlebotEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: TurtlebotEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.threshold = 0.1
        self.rew_scale_prog  = 3.0   
        self.rew_scale_goal  = 8.0 
        self.rew_scale_head  = 0.1
        ...
        self.max_dist = torch.zeros((self.num_envs), device=self.device, dtype=torch.float32)
        self.global_step = 0
        self.total_steps   = cfg.curriculum.total_steps
        self.progress    = 0.0 
        self.safe_dist = 0.25

        # joint id 매칭
        self._wheel_dof_idx, _ = self.robot.find_joints(["wheel_left_joint", "wheel_right_joint"])
    def _setup_scene(self):

        # ground plane, robot, object 삽입
        spawn_ground_plane(
            prim_path="/World/ground",
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.global_step += 1
        self.progress = min(1.0, self.global_step / self.total_steps)

        # 선속도/각속도 최대값을 상속
        v_scale = self.cfg.MAX_LIN
        w_scale = self.cfg.MAX_ANG

        # 급격한 변화를 방지하기 위해 선속도/각속도 클랭핑
        self._v_cmd = torch.clamp(actions[:, 0] * v_scale, -self.cfg.MAX_LIN,  self.cfg.MAX_LIN)   # [m/s]
        self._w_cmd = torch.clamp(actions[:, 1] * w_scale, -self.cfg.MAX_ANG,  self.cfg.MAX_ANG)   # [rad/s]
    ...
    def _apply_action(self) -> None:
        ...
        # joint id를 통해 로봇에 속도 적용
        self.robot.set_joint_velocity_target(wheel_vel, joint_ids=self._wheel_dof_idx)
    ...
    def _get_observations(self) -> dict:
        ...
        self.n_dist = self.dist / self.max_dist
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        prog_rew = torch.clamp(((self.prev_dist - self.dist) / self.max_dist), min=0.0)
        ...
        rew = (
            ...
            + self.goal_reached.float() * self.rew_scale_goal
        )
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # 커리큘럼 적용(0~0.2~0.8~1)
        warmup = 0.2 * self.total_steps
        ramp  = 0.6 * self.total_steps   # 0.2→0.8 구간

        if self.global_step < warmup:
            scale = 0.0
        elif self.global_step < warmup + ramp:
            scale = (self.global_step - warmup) / ramp      # 0→1 선형
        else:
            scale = 1.0                    # 이후 풀 난이도

        # 환경 원점에 대해 초기화 & 최대 0.1 이내로 랜덤화
        root_pose[:, :3] += self.scene.env_origins[env_ids]
        root_pose[:, 0] += torch.empty(len(env_ids), device=self.device).uniform_(-0.1 * scale, 0.1 * scale)
        root_pose[:, 1] += torch.empty(len(env_ids), device=self.device).uniform_(-0.1 * scale, 0.1 * scale)

        # 초기 각도 랜덤화(최대 ±25°)
        rand_yaw = (torch.rand(len(env_ids), device=self.device) - 0.5) * 1
        ...
        # 목표 지점 초기화 & 최대 0.3 이내로 랜덤화하여 visualize
        self.goal_pos[env_ids, 0] = self.cfg.goal_x
        self.goal_pos[env_ids, 1] = self.cfg.goal_y
        self.goal_pos[env_ids, 0] += torch.empty(len(env_ids), device=self.device).uniform_(-0.3 * scale, 0.3 * scale)
        self.goal_pos[env_ids, 1] += torch.empty(len(env_ids), device=self.device).uniform_(-0.3 * scale, 0.3 * scale)
        ...
        delta = self.goal_pos - self.robot.data.root_pos_w[:, :2]
        self.dist = torch.norm(delta, dim=-1)
        self.max_dist[env_ids] = self.dist[env_ids]
        self.prev_dist = self.dist.clone()
        heading_robot = self.robot.data.heading_w[:]
        heading_goal = torch.atan2(delta[:, 1], delta[:, 0])
