...
class TurtlebotEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # 목표 지점 초기화 & 0.1 이내로 랜덤화하여 visualize
        self.goal_pos[env_ids, 0] = self.cfg.goal_x
        self.goal_pos[env_ids, 1] = self.cfg.goal_y
        self.goal_pos[env_ids, 0] += torch.empty(len(env_ids), device=self.device).uniform_(-0.1, 0.1)
        self.goal_pos[env_ids, 1] += torch.empty(len(env_ids), device=self.device).uniform_(-0.1, 0.1)
