...
class TurtlebotEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # 목표 지점 병렬화하고 시각화 마커 삽입
        origins2d = self.scene.env_origins[:, :2]
        offset = torch.tensor([self.cfg.goal_x, self.cfg.goal_y], device=self.device)
        self.goal_world = origins2d + offset.unsqueeze(0)
        VisualizationMarkers(self.cfg.goal_cfg).visualize(
            translations=self.scene.env_origins + torch.cat((offset, torch.tensor([0.05], device=self.device))).unsqueeze(0)
        )
    ...
    def _get_observations(self) -> dict:
        ...
        delta = self.goal_world - self.robot.data.root_pos_w[:, :2]
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # 환경 원점에 대해 초기화 & 0.1 이내로 랜덤화
        root_pose[:, :3] += self.scene.env_origins[env_ids]
        root_pose[:, 0] += torch.empty(len(env_ids), device=self.device).uniform_(-0.1, 0.1)
        root_pose[:, 1] += torch.empty(len(env_ids), device=self.device).uniform_(-0.1, 0.1)

        # 초기 각도 랜덤화(±25°)
        rand_yaw = (torch.rand(len(env_ids), device=self.device) - 0.5) * 0.5
